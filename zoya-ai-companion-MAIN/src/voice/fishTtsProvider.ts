...
export class FishTTSProvider implements TTSProvider {
  ...
  constructor(apiKeyOverride?: string, voiceReferenceId: string = 'cbe13152c7ff4da98be9a95d448a1f39') { this.apiKeyOverride = apiKeyOverride; this.voiceReferenceId = voiceReferenceId; }
  ...
  getQuotaStatus():TTSQuotaStatus{const now=Date.now(); if(now<this.quotaCooldownUntil)return{exceeded:true,cooldownRemainingSec:Math.ceil((this.quotaCooldownUntil-now)/1000)}; return{exceeded:false,cooldownRemainingSec:0};}
  setApiKey(apiKey?:string){this.apiKeyOverride=apiKey;}
  setVoiceReferenceId(referenceId:string){this.voiceReferenceId=referenceId;}
}
