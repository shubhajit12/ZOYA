 ...
 *
 * The VRM root is rotated 180° by the loader so the character faces ZOYA's
 * camera. Gesture targets are therefore derived from the live character frame
 * and then converted back into each bone's local/rest-pose space.
 *
 * Important: Carlotta's authored arm/torso axes are not the same as the
 * character's visual forward axis. We explicitly calibrate the visual target
 * polarity from the loaded hierarchy instead of treating +Z/-Z as an
...
export class CarlottaGestureController{
 ...
 private calibrateWorldDirections(root:THREE.Object3D):void{
  this.pointOffsets.clear();this.bowOffsets.clear();root.getWorldQuaternion(_rootWorld);
  // The loader's root rotation is a 180° correction. The visual face direction
  // is the camera-facing direction. Use +Z in the final ZOYA frame explicitly.
  ...
  // Forward bow = rotate the torso around the NEGATIVE character-right axis.
  // For this model that is the opposite polarity of the previous implementation.
  ...
  if(isDevBuild())console.info('[CarloGestureCalibration] camera-facing forward polarity +Z; bow polarity corrected to negative character-right');
 }
 ...
 public reset():void{for(const bone of this.bones.values())bone.node.quaternion.copy(bone.base);this.bones.clear();this.pointOffsets.clear();this.bowOffsets.clear();this.initialized=false;this.active=null;this.phase='idle';this.time=0;this.blendT=1;this.recoverT=1;this.completed=null;carlottaAnimationController.setBodyHold(false);}
 ...
 private sample(def:GestureDef,t:number):void{const frames=def.keyframes;let a=frames[0],b=frames[frames.length-1];for(let i=1;i<frames.length;i+=1){if(t<=frames[i].time){b=frames[i];a=frames[i-1];break;}}const span=Math.max(.0001,b.time-a.time),w=smoothstep((t-a.time)/span);for(const bone of this.bones.values()){if(this.active==='point'){const offset=this.pointOffsets.get(bone.name);if(offset){const e=this.pointEnvelope(t);bone.sampled.set(offset.x*e,offset.y*e,offset.z*e);continue;}}if(this.active==='bow'){const offset=this.bowOffsets.get(bone.name);if(offset){const e=this.bowEnvelope(t);bone.sampled.set(offset.x*e,offset.y*e,offset.z*e);continue;}}const av=this.poseValue(a.pose,bone.name),bv=this.poseValue(b.pose,bone.name);const ax=av?av[0]:0,ay=av?av[1]:0,az=av?av[2]:0,bx=bv?bv[0]:0,by=bv?bv[1]:0,bz=bv?bv[2]:0;bone.sampled.set(ax+(bx-ax)*w,ay+(by-ay)*w,az+(bz-az)*w);}}
 private pointEnvelope(t:number):number{if(t<.32)return smoothstep(t/.32);if(t<1.45)return 1;if(t<2.15)return 1-smoothstep((t-1.45)/.70);return 0;} private bowEnvelope(t:number):number{if(t<.72)return smoothstep(t/.72);if(t<1.50)return 1;return 1-smoothstep((t-1.50)/.80);}
 public update(delta:number):void{if(isDevBuild()){this.diagAcc+=delta;if(this.diagAcc>=2){this.diagAcc=0;console.info(`[CarloGestDiag] init=${this.initialized} active=${this.active} phase=${this.phase} bones=${this.bones.size}`);}}if(!this.initialized||!this.active)return;const dt=Math.max(0,Math.min(delta,CARLOTTA_IDLE_MAX_DELTA)),def=GESTURES[this.active];if(this.phase==='active'){this.time+=dt;this.blendT=Math.min(1,this.blendT+dt/START_BLEND);const blend=smoothstep(this.blendT);this.sample(def,Math.min(this.time,def.duration));for(const bone of this.bones.values()){bone.current.lerpVectors(bone.from,bone.sampled,blend);_euler.set(bone.current.x,bone.current.y,bone.current.z);_quat.setFromEuler(_euler);bone.node.quaternion.copy(bone.base).multiply(_quat);}if(this.time>=def.duration){this.phase='recovering';this.recoverT=0;for(const bone of this.bones.values())bone.from.copy(bone.current);}return;}this.recoverT=Math.min(1,this.recoverT+dt/RECOVER_BLEND);const w=1-smoothstep(this.recoverT);for(const bone of this.bones.values()){bone.current.copy(bone.from).multiplyScalar(w);_euler.set(bone.current.x,bone.current.y,bone.current.z);_quat.setFromEuler(_euler);bone.node.quaternion.copy(bone.base).multiply(_quat);}if(this.recoverT>=1){const done=this.active;this.active=null;this.phase='idle';this.completed=done;for(const bone of this.bones.values()){bone.current.set(0,0,0);bone.from.set(0,0,0);bone.sampled.set(0,0,0);bone.node.quaternion.copy(bone.base);}carlottaAnimationController.setBodyHold(false);}}
 private registerDevHooks():void{if(!isDevBuild())return;try{const w=window as unknown as Record<string,unknown>;w.__carlottaGestureStart=(name:unknown):boolean=>isValidCarlottaGestureName(name)&&this.start(name);w.__carlottaGestureInfo=():{active:CarlottaGestureName|null;phase:GesturePhase;completed:CarlottaGestureName|null}=>({active:this.active,phase:this.phase,completed:this.completed});w.__carlottaGestureCancel=():void=>this.cancel();}catch{/* non-browser runtime */}}
}
