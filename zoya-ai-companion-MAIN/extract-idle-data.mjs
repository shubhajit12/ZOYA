...
const OUT_PATH = 'src/mint/idleMotionData.ts';
...
for (const ch of data.channels) {
  ...
  for (let tick = 0; tick <= totalTicks; tick += SAMPLE_STEP) {
    // Find the keyframe at this tick (linear search is fine for 299 entries)
    let key = rotKeys[0];
    ...
    // Compute delta quaternion: rest_inverse * current
    // For unit quaternions: inverse = conjugate = (w, -x, -y, -z)
    const dx = key.x - rest.x;
    const dy = key.y - rest.y;
    const dz = key.z - rest.z;
    const dw = key.w - rest.w;

    // For small deltas near identity, we can approximate
    // But let's compute the full delta properly
    // delta = conjugate(rest) * key
  ...
  // Check if scale varies
  const scaleKeys = ch.scaling_keys || [];
  let scaleVaries = false;
  if (scaleKeys.length > 1) {
    const s0 = scaleKeys[0];
    for (let i = 1; i < scaleKeys.length; i++) {
      if (Math.abs(scaleKeys[i].x - s0.x) > 0.001 ||
          Math.abs(scaleKeys[i].y - s0.y) > 0.001 ||
          Math.abs(scaleKeys[i].z - s0.z) > 0.001) {
        scaleVaries = true;
        break;
      }
    }
  ...
  const scaleInfo = scaleVaries ? ' + SCALE' : '';
  console.log(`  ${marker} ${part} (${boneName}): maxDev=${maxDev.toFixed(4)} (${(maxDev * 180 / Math.PI).toFixed(2)}°) keys=${rotKeys.length}${posInfo}${scaleInfo}`);
}
 ...
 *
 * GENERATED by extract-idle-data.mjs — DO NOT EDIT BY HAND.
...
export interface BoneSample {
  /** Time in ticks */
  t: number;
  /** Delta quaternion w component */
  qw: number;
  /** Delta quaternion x component */
  qx: number;
  /** Delta quaternion y component */
  qy: number;
  /** Delta quaternion z component */
  qz: number;
}

export interface BoneMotionData {
  /** Mixamo bone name for reference */
  mixamoBone: string;
  /** Mint bone key (Bip001 name suffix) */
  mintKey: string;
  /** Sampled delta quaternions */
  samples: BoneSample[];
  /** Maximum angular deviation from rest pose (radians) */
  maxDev: number;
}

/**
 * All animated bones with meaningful motion.
 * Delta quaternions are computed as: conjugate(restPose) * animatedPose
 */
...
for (const part of Object.keys(boneResults).sort()) {
  ...
  // Determine the Mint bone key
  let mintKey = '';
  // Map conceptual part names to actual Bip001 bone names
  const mintKeyMap = {
    'Hips': 'hips',
    'Spine': 'spine',
    'Spine1': 'spine1',
  ...
  mintKey = mintKeyMap[part] || part;

  // Filter out rest-pose samples (near identity) to save space
  // But keep them all for accuracy
