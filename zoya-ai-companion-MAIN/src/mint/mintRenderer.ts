...
const COMPANION_MODEL_Y_OFFSET = -1.60;
...
export class MintRenderer {
  ...
  private initEyeTracking() {
    if (!this.eyeTracker) this.eyeTracker = new EyeTracker();
    if (this.fbxAvatar) this.eyeTracker.init(this.fbxAvatar.group);
  }

  public getIsVrmLoaded(): boolean { return this.isVrmLoaded; }
  public getIsFbxLoaded(): boolean { return this.isFbxLoaded; }
  public getVrmAvatar(): LoadedCarlottaModel | null { return this.vrmAvatar; }
  ...
  public setEmotion(emotion: EmotionType, intensity = 0.45) {
    ...
    this.currentEmotionIntensity = Math.max(0, Math.min(1, intensity));
    if (this.vrmAvatar) carlottaExpressionController.setEmotion(emotion, this.currentEmotionIntensity);
  }

  public setSpeaking(isSpeaking: boolean) {
    this.appSpeaking = isSpeaking;
    this._lipsyncSpeaking = isSpeaking;
  }
  ...
  public getAnimationState(): string {
    if (this.vrmAvatar) return carlottaAnimationController.getState();
    return this.animController.getState();
  }

  public triggerAnimation(intent: AnimationIntent) {
    if (this.vrmAvatar) {
      carlottaGestureController.trigger(intent);
      return;
    }
    this.animController.trigger(intent);
  }

  public dispose() { this.unmount(); }

  private startRenderLoop() {
    const tick = (timestamp: number) => {
      this.animationFrameId = requestAnimationFrame(tick);
      const delta = Math.min(this.clock.getDelta(), 0.05);
      this.framePacer.update(delta);
      if (!this.framePacer.shouldRender()) return;
      if (this.vrmAvatar) {
        ...
        carlottaLipSync.update(delta, this.appSpeaking);
        this.vrmAvatar.vrm.update(delta);
      } else if (this.fbxAvatar) {
        this.animController.update(delta);
        if (this.eyeTracker) this.eyeTracker.update(delta, this.mouseX, this.mouseY, this.mouseOnCanvas);
        if (this.lipSync) this.lipSync.update(delta, this._lipsyncSpeaking);
      } else if (this.proceduralAvatar) {
        this.proceduralAvatar.update(delta);
      }
