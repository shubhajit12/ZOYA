...
fn enter_companion(app: tauri::AppHandle) -> Result<(), String> {
    ...
    if let Some(existing) = app.get_webview_window("companion") {
        ...
        let taskbar_edge = get_windows_taskbar_rect().map(|taskbar| {
            ...
            if taskbar.top >= work_bottom { "bottom" }
            else if taskbar.bottom <= work_top { "top" }
            else if taskbar.left >= work_right { "right" }
            else if taskbar.right <= work_left { "left" }
            else { "bottom" }
        }).unwrap_or("bottom");
...
fn finish_companion_drag(app: tauri::AppHandle) -> Result<bool, String> {
    ...
    if engine_running {
        ...
        companion.set_position(tauri::Position::Physical(tauri::PhysicalPosition::new(x, y)))
            .map_err(|e| e.to_string())?;
    }
