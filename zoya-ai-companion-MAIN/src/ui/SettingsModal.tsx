...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const [settingsSection, setSettingsSection] = useState<'general' | 'minecraft'>('general');
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#050506]/85 backdrop-blur-xl">
      <div className="w-full max-w-5xl glass border border-white/10 rounded-3xl shadow-2xl overflow-hidden glow-amber max-h-[92vh]">
        <div className="flex items-center justify-between border-b border-white/10 px-6 py-4">
          <h2 className="text-lg font-bold text-slate-100 flex items-center gap-2"><Monitor className="w-5 h-5 text-orange-400" /><span className="tracking-wide">Settings & Preferences</span></h2>
          <button onClick={onClose} className="p-1.5 rounded-xl glass text-slate-400 hover:text-white hover:bg-white/10 transition-colors"><X className="w-5 h-5" /></button>
        </div>
        <div className="flex min-h-[620px] max-h-[calc(92vh-70px)]">
          <aside className="w-56 flex-shrink-0 border-r border-white/10 bg-black/10 p-3">
            <p className="px-3 py-2 text-[10px] font-bold uppercase tracking-[0.18em] text-slate-500">Settings</p>
            <button type="button" onClick={() => setSettingsSection('general')} className={settingsSection === 'general' ? 'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs font-semibold bg-orange-500/15 text-orange-300 border border-orange-500/20' : 'w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs font-semibold text-slate-400 hover:text-slate-200 hover:bg-white/5'}><Monitor className="w-4 h-4" /> General</button>
            <button type="button" onClick={() => setSettingsSection('minecraft')} className={settingsSection === 'minecraft' ? 'w-full mt-1 flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs font-semibold bg-orange-500/15 text-orange-300 border border-orange-500/20' : 'w-full mt-1 flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs font-semibold text-slate-400 hover:text-slate-200 hover:bg-white/5'}><Gamepad2 className="w-4 h-4" /> Minecraft</button>
          </aside>
